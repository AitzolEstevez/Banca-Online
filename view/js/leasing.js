...
function getTasa(tasa1, tasa_tipo2, periodo2) {
    if (tasa_tipo2 == ANUAL) { tasa1 = tasa1 / 12 } tasa1 = tasa1 / 100.0
...
function getValorDeCuotaFija(monto1, tasa1, cuotas1, periodo2, tasa_tipo2) {
    tasa1 = getTasa(tasa1, tasa_tipo2, periodo2); valor2 = monto1 * ((tasa1 * Math.pow(1 + tasa1, cuotas1)) / (Math.pow(1 + tasa1, cuotas1) - 1));
...
function getAmortizacion(monto1, tasa1, cuotas1, periodo2, tasa_tipo2) {
    ...
    //var Deuda = saldo_al_capital1-residual; 
    var items2 = new Array(); 
    valor_de_cuota1=parseFloat(valor_de_cuota1); // console.log("saldo_al_capital"); // console.log(saldo_al_capital) 
    for (i=0; i < cuotas1; i++) { 
    interes1 = saldo_al_capital1 * getTasa(tasa1, tasa_tipo2, periodo2); // console.log("valor_de_cuota"); // console.log(valor_de_cuota); 
    var abono_al_capital1 = valor_de_cuota1 - interes1; // 
    console.log("abono_al_capital");
    console.log(abono_al_capital1);
    saldo_al_capital1 -= abono_al_capital1; 
    IVA = (valor_de_cuota1*21)/100; // console.log("1") // console.log(abono_al_capital) 
    ...
    IVA = parseFloat(IVA);
    cuotaT = valor_de_cuota1 + IVA; // console.log(valor_de_cuota1) // console.log(IVA) // console.log(cuotaT) 
    numero1 = i+1; 
    interes1 = interes1.toFixed(2); abono_al_capital1 = abono_al_capital1.toFixed(2);
    saldo_al_capital1 = saldo_al_capital1.toFixed(2);
    item2 = [numero1, valor_de_cuota1, IVA, cuotaT, interes1, abono_al_capital1, Deuda]; items2.push(item2);
...
function getAmortizacion(monto1, tasa1, cuotas1, periodo2, tasa_tipo2) {
    ...
    var items2 = new Array(); valor_de_cuota1 = parseFloat(valor_de_cuota1);
    // console.log("saldo_al_capital"); 
    // console.log(saldo_al_capital) 
    for (i = 0; i < cuotas1; i++) {
        interes1 = saldo_al_capital1 * getTasa(tasa1, tasa_tipo2, periodo2);
        // console.log("valor_de_cuota"); // console.log(valor_de_cuota);
        abono_al_capital1 = valor_de_cuota1 - interes1;
        // saldo_al_capital -= abono_al_capital; 
        IVA = (valor_de_cuota1 * 21) / 100;
        // console.log("1") // console.log(abono_al_capital) 
        saldo_al_capital1 = parseFloat(saldo_al_capital1);
        abono_al_capital1 = parseFloat(abono_al_capital1);
        Deuda = (saldo_al_capital1 - abono_al_capital1);
        Deuda = Deuda.toFixed(2); IVA = parseFloat(IVA);
        cuotaT = valor_de_cuota1 + IVA;
        // console.log(valor_de_cuota1) // console.log(IVA) // console.log(cuotaT) 
        numero1 = i + 1;
...
function setMoneda(num) {
    ...
    for (var i = 0; i < Math.floor((num.length - (1 + i)) / 3); i++) num = num.substring(0, num.length - (4 * i + 3)) + ',' + num.substring(num.length - (4 * i + 3)); return (((sign) ? '' : '-') + '$' + num + ((cents == "00") ? '' : '.' + cents));
}
function calcularLeasing() {
    ...
    var tbody1 = document.getElementById("tbody_2"); tbody1.innerHTML = "";
    if (parseInt(cuotas1) > 3000) {
        alert("Ha indicado una cantidad excesiva de cuotas, porfavor reduzcala a menos de 3000");
        return;
    }
    for (i = 0; i < items2.length; i++) {
        item2 = items2[i]; tr2 = document.createElement("tr");
        for (e = 0; e < item2.length; e++) {
            value2 = item2[e];
            if (e > 0) { value2 = setMoneda(value2); }
            td2 = document.createElement("td");
            textCell2 = document.createTextNode(value2); td2.appendChild(textCell2); tr2.appendChild(td2);
        } tbody1.appendChild(tr2);
    }
    var div1 = document.getElementById("div-valor-cuota1"); 
    ...
    var div2 = document.getElementById("div-comentario1");
    div2.innerHTML = msg; 
}
